...
| 映像內容 | 後果 |
|---|---|
`data/raw` **只放** 132 份非營利財報原件（1.2 GB） | 證據頁與文件控管室原件頁的截圖在雲端看得到。代價是主辦方資料集（不得轉散布）放在公開網址後面，快速登入開著時任何人都看得到——2026-09-13 使用者知情下決定。其餘原始資料（公校決算書等）不進映像，截圖仍是 404 |
`data/runtime`（SQLite）不進映像 | 容器重啟會失去帳號與對話稽核軌跡。要保留就接 RDS（見 §四） |
...
- **映像裡有 1.2 GB 的財報原件**（證據頁截圖要用），建置、推送、Fargate 拉取都比較久。
  `.dockerignore` 只放行 `data/raw/資料集/非營利園財報`；其餘原始資料不進映像，截圖仍是 404。
